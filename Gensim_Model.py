...
class Gensim_Model():
    ...
    def evaluate_several_models(self, algorithms = [TopicModelingAlgorithm.LDA, TopicModelingAlgorithm.LSA, TopicModelingAlgorithm.NMF], qtt_topics = range(2,21), topns = [5], out_folder=None, out_file=None):
        ...
        if(out_folder != None):
            out_folder_exists = os.path.isdir(out_folder)
    ...
    def cur_model_top_topics_by_coh(self):
        c = self.__corpus
        idw2 = self.__id2words
        d_l = self.__data_lemmatized
        return self.__current_model.top_topics(c, d_l, idw2, coherence='c_v', topn=5)
